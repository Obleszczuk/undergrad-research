...
# generates a high-resolution 2D lateral distribution plot (histogram) of the particle positions,
# applies a zoom to the central region of the shower, and uses a logarithmic color scale.
...
def gerar_grafico_distribuicao(coords_x, coords_y, nome_saida, zoom_limite, bins):
    if not coords_x:
        print("Nenhuma partícula encontrada para gerar o gráfico.")
        return

    print(f"Gerando o gráfico 2D com zoom de {zoom_limite}m e {bins}x{bins} bins...")
    ...
    hist = ax.hist2d(coords_x_m, coords_y_m, bins=bins, cmin=1, cmap=plt.cm.jet,
                       norm=LogNorm())

    fig.colorbar(hist[3], ax=ax, label='Contagem de Partículas por Área (escala log)')

    ax.set_title(f'Distribuição Lateral do Chuveiro (Zoom {zoom_limite/1000}km)', fontsize=16)
    ...
    ax.set_xlim(-zoom_limite, zoom_limite) 
    ax.set_ylim(-zoom_limite, zoom_limite) 
...
if __name__ == "__main__":
    ...
    if x is not None:
        gerar_grafico_distribuicao(x, y, nome_arquivo_saida, limite_zoom_m, numero_de_bins)
